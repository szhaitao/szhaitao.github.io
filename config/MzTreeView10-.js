...
MzTreeView.prototype.showNode = function(e)
{
...
{
	alert("findByHint:"+nid);
	n=this.node[nid];
//	alert(this.node.length);
//	for (i=n.sourceIndex+1; i<this.node.length; i++)
//		if (this.nodes[i].hint==str)
//		{
//			alert(this.nodes[i].text);
//			this.showNode(this.nodes[i]);
//			this.focusClientNode(this.nodes[i].id);
//			return;
//		}
	while (n)
	{
		alert("findByHint.while:"+n.text);
		n=this.getNextNode(n);
		alert("22:"+n.text);
		if (n.hint==str)
		{
			alert("Find!"+n.text);
			this.showNode(n);
			this.focusClientNode(n.id);
			return;
		}
	}
}
...
MzTreeView.prototype.expandnode = function(L,bExp)
{
	if (L==1)
	{
		if (!this.currentNode)
			return 0;
		this.expandnodeself(this.currentNode,bExp);
		return 1;
	}
	if (L==2)
	{
		this.expandnodesub(this.currentNode,bExp);
		return 1;
	}
	if (L==3)
	{
		this.expandnodesub(this.node["0"],bExp);
		return 1;
	}
}
...
//this.expandnode(3,false);
  ...
  this.onkeydown= function(e)
  {
    ...
    switch(key)
    {
      case 37 : eval(Tname).upperNode(); e.keyCode=0; break;  //Arrow left, shrink child node
      case 38 : eval(Tname).pervNode();  e.keyCode=0; break;  //Arrow up
      case 39 : eval(Tname).lowerNode(); e.keyCode=0; break;  //Arrow right, expand child node
      case 40 : eval(Tname).nextNode();  e.keyCode=0; break;  //Arrow down
    }
  };
}
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  this.node[this.index] =
  {
    ...
    ,"level" : (parentId=="")?0:(this.node[parentId].level+1)
  };
